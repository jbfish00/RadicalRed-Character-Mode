...
1. ~~Commit initial scaffold~~ DONE. ~~CFRU-as-structural-donor bet~~ DONE. ~~Seed-list reconciliation~~ DONE. ~~Enforcement-hook anchor searches~~ done for catch/PC/Mystery-Gift tutorial; trade still open. ~~Species-ID index range~~ SOLVED. ~~Evolution table~~ SOLVED. ~~map_species.py~~ DONE, 100% resolved. ~~emit_characters.py~~ DONE — Phase 2 complete, 184/184 characters emitted to flat binaries.
2. **Phase 3 (sprites)**: locate Radical Red's overworld/trainer-front/trainer-back pic tables (naming-convention auto-match like ROWE's `const_candidates()`, or manual RE) so `sprite_asset_id` can move off its `0xFFFF` placeholder. Not yet started.
...
5. Sprite table locations (see #2) and the intro-menu hook point remain unsearched.
6. **Phase 4 design note (carried forward)**: injected enforcement C code can use plain numeric species ids directly (or a small generated `#define` header) since `rosters.bin`'s ids are already the real ROM base-stats table indices — no need to round-trip through CFRU's `species.h` at all, including for the ~82-entry range it doesn't cover.
7. Once Phase 1's hook sites are confirmed (items 3-5), write the ROM insert step: allocate `characters.bin`/`rosters.bin`/`names.bin` into the confirmed free-space blocks (`docs/FREE_SPACE.md`), patch real `0x08xxxxxx` pointers over the manifest's blob-relative offsets, and wire the enforcement hooks found in items 3-4.

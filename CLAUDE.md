...
1. ~~Everything through v10~~ DONE. ~~Catch hook (dispatch table → atkF0_givecaughtmon → GiveMonToPlayer)~~ **CONFIRMED — Phase 1's biggest gate item closed.**
2. **Find `ScriptGiveMon`'s compiled address** — run `find_pointer_refs.py` for `GiveMonToPlayer`'s Thumb address value (`0x0907D791`) to find its callers' literal pools, then `CreateAndDecompile.java` each caller and match against CFRU's `src/build_pokemon.c` source. Closes the gift/static-mon delivery hook the same way the catch hook fell.
3. Empirically re-test the sprite/trainer-pic hardcoded addresses (`gTrainerFrontPicTable` = `0x823957C` etc.) — the v10/v11 sessions proved these compile-time constants often hold; read the raw bytes and check for `CompressedSpriteSheet`-shaped records before assuming relocation.
4. Find a genuine in-game NPC trade dialogue anchor and a Mystery-Gift delivery-relevant anchor — `search_gametext.py`/`dump_all_strings.py`, still open.
5. Intro-menu hook point — unsearched. (Radical Red's equivalent of Unbound's "enhancement options" intro menu — RR has a similar options flow at new-game per community docs; find its script/flag writes.)
6. **Phase 4 design note (carried forward)**: injected enforcement C can use plain numeric species ids directly (`rosters.bin` ids = real ROM base-stats indices), vanilla pret/pokefirered RAM symbols (confirmed unmoved), and must respect the Battle-Tower-rental caveat on hooking `GiveMonToPlayer` (see `docs/CFRU_CROSSWALK.md`).
7. Once items 2/5 close, write the ROM insert step: allocate the Phase 2 blobs into confirmed free space, patch pointers, wire the enforcement hooks.
8. Sprite donor-PNG resolution + `CREDITS.md` once item 3 resolves the table addresses.

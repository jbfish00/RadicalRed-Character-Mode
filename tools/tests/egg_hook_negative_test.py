...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        def bend_goto(d):
            struct.pack_into("<I", d, egg_hook.SPLICE_FILE_OFF + 1, 0x08C8F100)
        ...
        def bend_native(d):
            struct.pack_into("<I", d, EGG_TAIL_OFF + 6, 0x08C80201)
        ...
        def reorder(d):
            ...
            d[EGG_TAIL_OFF:EGG_TAIL_OFF + 11] = (
                ...
                + bytes([0x27, egg_hook.OPCODE_RELEASE, 0x02]))
    ...
    if fails:
        ...
        return 1
    print("ALL PASS")
    if assert_cases(passes + len(fails), EXPECT_CASES, 'egg_hook'):
        return 1
    return 0

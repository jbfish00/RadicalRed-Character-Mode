...
SIBLING_RE = re.compile(r"(?:Unbound|Seaglass|Prism|Platinum|Lazarus)"
                        r"-Character-Mode")
...
_SURVEY = ("cross-repo SURVEY tool by design: reads every sibling's live "
           ...
           "Seaglass and Prism) to score engine-native art. This is the "
           "canonical copy for the whole workspace. Never on the build or "
           "verify path.")
...
ALLOWED_SIBLING = {
    ...
    "tools/stage_donor_art.py":
        "cross-repo DONOR tool by design: stages sprite art across the sibling "
        "repos. Never on the build or verify path.",
